...
위는 선형 색상 곡선과 로그 색상 곡선 사이의 차이를 나타낸 것이다.    
곡선에 익숙하다면, 곡선의 왼쪽 아래가 검정과 그림자를 나타낸다는 것을 알 수 있다. 그리고 곡선의 오른쪽 위로 이동할 때, 하이라이트와 화이트를 볼 수 있다. 보다시피 로그 곡선은 그림자를 유지하기 위해 이미지의 어두운 부분을 위로 민다. 하이라이트를 유지하기 위해 곡선의 상단이 아래로 이동합니다. 따라서 각 색상 곡선의 측면에서 더 많은 데이터를 보존할 수 있다.

선형 색 공간은 장면에서 정확한 수학적 값에 대해 일정하고 변경되지 않는 휘도 값을 가진다. 인간의 눈에는, 우리의 눈(그리고 그 물질에 대한 모니터)이 정확한 휘도 값과 다른 색을 보고 있고 밝고 어두운 영역에서 더 많은 세부 사항을 볼 수 있기 때문에, 특정 영역에서는 매우 어둡고 진흙으로 보일 것이다. 따라서 색 정보를 보다 미적으로 만족스럽게 볼 수 있도록 다른 색 공간을 적용하고 표시 장치와 모니터의 색 값을 표준화해야 합니다.
